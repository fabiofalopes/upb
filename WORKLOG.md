...
| 2026-09-17 | gas-router phase0→1 | G0 closure: merged `queue-doctrine` → `tokenrouter-lane+failclosed` (clean auto-merge; queue gate + fail-closed adapter + capability-derived /v1/models all present). Fixed env-dependent failover E2E (pf-screen daemon absent on workstations → fail-closed 503 before the mock 429; test now sets `PF_SCREEN_DISABLE=1` pre-import). 14/14 tests green. Push path fixed: https store cred is 403-denied → origin switched to SSH (verified); added `lxc165` remote (`ssh://root@192.168.80.10/opt/upb`, deploy key authorized on the LXC). Deploy runbook in `docs/DEPLOY.md`. Core decision logged: LiteLLM gateway core on LXC 165 (UPB demotes to subscription-wrapper appliance; pf-screen daemon survives). | `e44f203`, `873a1ed`, `539768e` |
