...
**Execution status:** P0 done 2026-08-17 — `future/` committed to panopticon (`c2ef3f3`, pushed) and vendored to `docs/reference/panopticon-future/`. P1-P3 implemented 2026-08-17 (pricing + `cost_usd`, provider cooldown/failover + first test suite, custom headers + Mistral provider) — see WORKLOG session log; pending Linux-box deploy + live validation. Follow-ups same day: Mistral model ids verified live and corrected (trio of `mistral-vibe-cli-*` subscription models + `-latest` platform ids); `upb verify` added as the standing catalog-audit command (listing diff + opt-in 1-token entitlement probe) — the direct answer to the stale-id bug class that started this. G10 closed: `amalia-9b` added to the litellm provider. P4 skipped (optional). P5 done 2026-08-17 — stack stopped (`docker compose down`, volumes kept), tag `archive-2026-08` pushed, GitHub description set to archived/superseded-by-upb. Remaining: deploy + live validation on the Linux box, migrate the working Mistral key into upb secrets. 2026-08-18: GitHub repo archived (read-only, reversible); README/HANDOFF vendored to `docs/reference/panopticon-archive/`; fabric `.env` repointed to `http://127.0.0.1:8705/v1` ahead of the pending local upb deployment (the Aug 16 "mystery Go client" was fabric itself — Go binary, OpenAI Go SDK UA, `DEFAULT_MODEL=codestral`). Late finding: one live consumer remained — a Go OpenAI-SDK client on the Mac called `codestral` via :4000 on 2026-08-16 ($0.0029); identify before it next needs an LLM. Mistral Vibe plan confirmed alive by that call.
